...
description: 在为新站点编写OpenCLI适配器或向现有站点添加新命令时使用。提供从首次侦测到现场解码、适配器编码和验证的端到端指南。取代opencli-oneshot / opencli-explorer。对于临时浏览器驱动（无适配器），请参阅opencli-browser；对于OpenCLI的顶级概述，请参阅opencli-usage。
