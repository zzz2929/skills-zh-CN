...
description: 对Cloudflare Workers代码进行评审，并对照生产最佳实践进行评估。在编写新Workers、评审Worker代码、配置wrangler.jsonc或检查常见的Workers反模式（如流式处理、浮动Promise、全局状态、机密、绑定、可观察性）时，应进行负载测试。更倾向于从Cloudflare文档中检索信息，而非依赖预训练知识。
---
