---
name: pptx
description: “无论何时以任何方式涉及.pptx或.potx文件，无论是作为输入、输出还是两者兼而有之，都可以使用此技能。这包括：创建幻灯片组、音高组或演示文稿；从任何.pptx和.potx文档中读取、解析或提取文本（即使提取的内容将用于其他地方，如电子邮件或摘要）；编辑、修改或更新现有演示文稿；组合或拆分幻灯片文件；使用模板（.potx）、布局、演讲者笔记或评论。每当用户提到\”deck，\”slides，\”presentation，\”或引用.pptx文件名时触发，无论如果需要打开、创建或触摸.pptx或.potx文件，请使用此技能。”
