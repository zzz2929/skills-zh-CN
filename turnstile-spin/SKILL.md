---
name: turnstile-spin
description: 在项目中设置Cloudflare Turnstile端到端流程——扫描代码库，通过Cloudflare API创建小部件，部署托管站点verify Worker，编写前端代码片段，验证并保存技能。当用户要求添加Turnstile、设置验证码、保护表单免受机器人攻击或修复Turnstile集成时，加载此技能。镜像开发者可访问developers.cloudflare.com/turnstile/spin。
references:
  - vanilla-html
  - nextjs-app
  - nextjs-pages
