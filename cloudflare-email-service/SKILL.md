...
description: 使用Cloudflare电子邮件服务（电子邮件发送+电子邮件路由）发送和接收事务性电子邮件。在构建电子邮件发送（Workers绑定或REST API）、电子邮件路由、Agents SDK电子邮件处理或将电子邮件集成到任何应用程序（Workers、Node.js、Python、Go等）时使用。也可用于电子邮件送达率、SPF/DKIM/DMARC、wrangler电子邮件设置、MCP电子邮件工具，或在编码代理需要发送电子邮件时使用。即使是像“将电子邮件添加到我的Worker”这样的简单请求，此技能也有关键的配置详细信息。
---
