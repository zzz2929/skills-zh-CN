---
name: web-perf
description: 使用Chrome DevTools MCP分析网页性能。测量核心网页关键指标（LCP、INP、CLS）和辅助指标（FCP、TBT、速度指数），识别阻碍渲染的资源、网络依赖链、布局变化、缓存问题和可访问性差距。在要求审核、分析、调试或优化页面加载性能、Lighthouse评分或网站速度时使用。偏向于从当前文档中检索，而非依赖预训练知识。
---
