...
description: 使用React的视图转换API（`<ViewTransition>`组件、`addTransitionType`和CSS视图转换伪元素）实现流畅、本机生成动画的指南。每当用户想要添加页面过渡、设置路线更改的动画、创建共享元素动画、设置组件的进入/退出的动画、设置列表重新排序的动画、实现定向（向前/向后）导航动画或在Next.js中集成视图过渡时，都可以使用此技能。当用户提到视图转换、`startViewTransition`、`ViewTransition'、转换类型或询问在没有第三方动画库的情况下在React中的UI状态之间设置动画时，也可以使用。
license: MIT
metadata:
  author: vercel
  version: "1.0.0"
