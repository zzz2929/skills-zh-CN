...
description: 当代理需要通过opencli驱动一个真实的Chrome窗口时使用——检查页面、填写表单、点击登录流程或临时提取数据。涵盖了选择器优先的目标契约、复合表单字段、过时引用处理、网络捕获以及CLI返回的代理原生封装。不适用于编写适配器——有关适配器编写，请参阅opencli-adapter-author。
