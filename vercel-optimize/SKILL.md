...
description: “用于已部署项目的Vercel成本和性能优化，特别是Next.js、SvelteKit、Nuxt和有限的Astro应用程序。首先收集Vercel指标、使用情况、项目配置和代码扫描结果；只调查指标支持的候选者；根据经验证的文件和版本感知的Vercel/framework文档生成排名建议。触发Vercel账单减少、慢速或昂贵的路由、缓存机会、功能调用、构建分钟、快速数据传输、核心Web生命、机器人管理、流体计算或成本分解请求。”
metadata:
  version: "1.2.0"
---
